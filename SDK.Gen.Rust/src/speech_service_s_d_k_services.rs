...
impl <C: TThriftClient + TSpeechServiceSyncClientMarker> TSpeechServiceSyncClient for C {
  fn say(&mut self, command: speech_service_s_d_k_data_types::VoiceCommand) -> thrift::Result<speech_service_s_d_k_data_types::SpeechResult> {
    (
      {
        ...
        let call_args = SpeechServiceSayArgs { command: command };
    ...
    )?;
    {
      ...
      let result = SpeechServiceSayResult::read_from_in_protocol(self.i_prot_mut())?;
      self.i_prot_mut().read_message_end()?;
      result.ok_or()
    }
  }
...
impl TSpeechServiceProcessFunctions {
  pub fn process_say<H: SpeechServiceSyncHandler>(handler: &H, incoming_sequence_number: i32, i_prot: &mut TInputProtocol, o_prot: &mut TOutputProtocol) -> thrift::Result<()> {
    let args = SpeechServiceSayArgs::read_from_in_protocol(i_prot)?;
    match handler.handle_say(args.command) {
      Ok(handler_return) => {
        ...
        let ret = SpeechServiceSayResult { result_value: Some(handler_return) };
...
//
// SpeechServiceSayArgs
//

#[derive(Clone, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
struct SpeechServiceSayArgs {
  command: speech_service_s_d_k_data_types::VoiceCommand,
}

impl SpeechServiceSayArgs {
  fn read_from_in_protocol(i_prot: &mut TInputProtocol) -> thrift::Result<SpeechServiceSayArgs> {
    ...
    verify_required_field_exists("SpeechServiceSayArgs.command", &f_1)?;
    let ret = SpeechServiceSayArgs {
...
//
// SpeechServiceSayResult
//

#[derive(Clone, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
struct SpeechServiceSayResult {
  result_value: Option<speech_service_s_d_k_data_types::SpeechResult>,
}

impl SpeechServiceSayResult {
  fn read_from_in_protocol(i_prot: &mut TInputProtocol) -> thrift::Result<SpeechServiceSayResult> {
    i_prot.read_struct_begin()?;
    ...
    i_prot.read_struct_end()?;
    let ret = SpeechServiceSayResult {
      result_value: f_0,
    };
    Ok(ret)
  }
  fn write_to_out_protocol(&self, o_prot: &mut TOutputProtocol) -> thrift::Result<()> {
    let struct_ident = TStructIdentifier::new("SpeechServiceSayResult");
    o_prot.write_struct_begin(&struct_ident)?;
      ...
      Err(
        thrift::Error::Application(
          ApplicationError::new(
            ...
            "no result received for SpeechServiceSay"
          )
        )
      )
    }
